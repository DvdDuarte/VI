...
bool Scene::trace_recursive(Ray r, Intersection* isect, int depth) {
    ...
    for (auto & light : lights) {
        // Check for intersection with the light source
        if (light->intersect(r, &curr_isect)) {
            if (!intersection || curr_isect.depth < isect->depth) {
                intersection = true;
                *isect = curr_isect;
                isect->isLight = true;
                isect->Le = light->L();  // assuming L() returns the light's emitted radiance
            }
        }
    }
    ...
    reflectedRay.dir = reflect(r.dir, isect->gn);  // use your reflect() function
